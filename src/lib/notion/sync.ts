...
import { desiredFiles, postPath, type RenderedPost } from "./plan";
...
// still referenced by the file kept on disk, but this run never downloaded them.
export function preservedImageDirs(plan: SyncPlan): string[] {
  return plan.preserved.map(imageDir);
}
